...
const Dashboard: React.FC<DashboardProps> = ({ isDarkMode }) => {
  ...
  const getTreatmentPhase = (time: string) => {
    ...
    if (hour < 10) return { label: 'Triage', color: 'bg-red-500', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' };
    if (hour < 12) return { label: 'Consultation', color: 'bg-red-600', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' };
    if (hour < 14) return { label: 'Procedure', color: 'bg-red-400', icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z' };
    return { label: 'Recovery', color: 'bg-red-700', icon: 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z' };
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex justify-between items-end">
        ...
        <div className={`px-4 py-2 rounded-2xl shadow-sm border flex items-center gap-2 group transition-all cursor-default ${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-red-500/50' : 'bg-white border-slate-100 hover:border-red-200'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Heart Rate', value: '72', unit: 'bpm', trend: '-2%', color: 'red', isLive: true, icon: 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z' },
          { label: 'Blood Pressure', value: '118/76', unit: 'mmHg', trend: 'Stable', color: 'red', isLive: false, icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
          { label: 'Avg Steps', value: '9,450', unit: 'steps', trend: '+12%', color: 'red', isLive: false, icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' },
          { label: 'Throughput', value: '88%', unit: 'eff', trend: '+5%', color: 'red', isLive: false, icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
        ].map((stat, i) => (
          <div key={i} className={`p-6 rounded-[2rem] shadow-sm border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
            <div className="flex justify-between items-start mb-4">
              <div className={`w-12 h-12 bg-red-50/10 text-red-500 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-3 shadow-inner border border-red-500/20`}>
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={stat.icon} />
                </svg>
              </div>
              {stat.isLive && (
                <div className="px-2 py-1 bg-red-50 rounded-lg flex items-center gap-1.5 border border-red-100 animate-pulse">
        ...
        <div className="p-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
           ...
           <div className="lg:col-span-3 space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
              {filteredAppointments.length > 0 ? filteredAppointments.map(app => {
                ...
                return (
                  <div key={app.id} className={`p-5 rounded-[2rem] border transition-all group flex items-center gap-6 ${isDarkMode ? 'bg-slate-950 border-slate-800 hover:border-red-500/40' : 'bg-slate-50 border-slate-100 hover:border-red-200 shadow-sm'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="p-2 bg-white/5 border border-slate-200/50 rounded-xl">
                          ...
                          <p className={`text-[10px] font-bold ${isDarkMode ? 'text-red-400' : 'text-red-600'}`}>98% Optimal</p>
                        </div>
                        <div className="p-2 bg-white/5 border border-slate-200/50 rounded-xl">
                    ...
                    <button className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isDarkMode ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-white text-slate-400 hover:text-red-600 shadow-sm border border-slate-100'}`}>
           ...
           <div className="space-y-6">
                 ...
                 <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-end mb-2">
                        ...
                        <span className={`text-xs font-black ${isDarkMode ? 'text-red-400' : 'text-red-600'}`}>12m</span>
                      </div>
                      <div className="w-full bg-slate-200/20 h-1.5 rounded-full overflow-hidden">
                        <div className="bg-red-500 h-full w-[12%] rounded-full shadow-[0_0_8px_rgba(239,68,68,0.5)]"></div>
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between items-end mb-2">
              ...
              <div className={`p-6 rounded-3xl border text-white relative overflow-hidden group ${isDarkMode ? 'bg-indigo-900/20 border-indigo-500/30' : 'bg-slate-900 border-slate-800'}`}>
                 ...
                 <h5 className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-3">Triage Suggestion</h5>
                 ...
                 <button className="mt-4 text-[9px] font-black text-white hover:text-red-400 uppercase tracking-widest underline decoration-red-500/50 underline-offset-4 transition-colors">Apply Strategy</button>
              </div>
           </div>
        </div>
      </section>
      {/* ... rest of analytics sections with updated charts if needed ... */}
    </div>
  );
};
