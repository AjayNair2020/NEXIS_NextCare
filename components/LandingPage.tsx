    ...
    <div className="min-h-screen bg-[#f0f9f1] text-slate-900 flex flex-col overflow-x-hidden relative">
      ...
      <nav className="relative z-10 px-8 py-6 flex justify-between items-center max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black tracking-tighter uppercase italic text-emerald-950">NextCare</h1>
        </div>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-8 pt-12 pb-24 max-w-6xl mx-auto text-center">
        <div className="inline-block px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 text-[10px] font-black uppercase tracking-[0.3em] mb-6">
          NEXIS 2.0 BIOMETRIC SYNC ACTIVE
        </div>
        <h2 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-6 text-emerald-950">
          SECURE <span className="text-emerald-500 italic">IDENTITY</span> <br />
          HEALTHCARE ACCESS.
        </h2>
        
        {/* Authentication Switcher */}
        <div className="flex bg-white/40 p-1 rounded-2xl border border-emerald-100 backdrop-blur-md mb-8">
          <button 
            onClick={() => setLoginMode('patient')}
            className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${loginMode === 'patient' ? 'bg-white text-emerald-600 shadow-xl' : 'text-emerald-800/40 hover:text-emerald-600'}`}
          >
            Patient Access
          </button>
          <button 
            onClick={() => setLoginMode('staff')}
            className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${loginMode === 'staff' ? 'bg-slate-900 text-white shadow-xl' : 'text-emerald-800/40 hover:text-emerald-600'}`}
          >
            Clinical Command
          </button>
        </div>
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          {/* Main Auth Panel */}
          <div className={`p-10 rounded-[3rem] border transition-all duration-500 text-left ${loginMode === 'staff' ? 'bg-slate-900 border-slate-800 text-white shadow-2xl' : 'bg-white/70 border-emerald-100 text-emerald-950 shadow-xl backdrop-blur-xl'}`}>
            <h3 className="text-xl font-black uppercase tracking-widest mb-2 italic">
              {loginMode === 'staff' ? 'Staff Portal' : 'Patient Login'}
            </h3>
            <p className={`text-sm mb-8 font-medium ${loginMode === 'staff' ? 'text-slate-400' : 'text-emerald-800/60'}`}>
            ...
            <div className="space-y-4">
              {loginMode === 'staff' ? (
                <button 
                  ...
                  className="w-full py-5 bg-emerald-500 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-4"
                >
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  {[
                    ...
                    <button 
                      ...
                      className="flex flex-col items-center justify-center p-4 bg-white border border-emerald-100 rounded-2xl hover:border-emerald-500 hover:shadow-lg transition-all group"
                    >
                      <svg className="w-6 h-6 text-emerald-600 group-hover:scale-110 transition-transform mb-2" fill="currentColor" viewBox="0 0 24 24">
                        <path d={soc.icon} />
                      </svg>
                      <span className="text-[9px] font-black uppercase text-emerald-800/40">{soc.name}</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="relative py-4 flex items-center">
                <div className={`flex-grow border-t ${loginMode === 'staff' ? 'border-slate-800' : 'border-emerald-100'}`}></div>
                <span className={`flex-shrink mx-4 text-[10px] font-black uppercase tracking-widest ${loginMode === 'staff' ? 'text-slate-600' : 'text-emerald-800/30'}`}>Or Traditional Sync</span>
                <div className={`flex-grow border-t ${loginMode === 'staff' ? 'border-slate-800' : 'border-emerald-100'}`}></div>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <input 
                  ...
                  className={`w-full border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 transition-all ${loginMode === 'staff' ? 'bg-slate-800 text-white placeholder:text-slate-600 focus:ring-emerald-500/20' : 'bg-white border-emerald-100 text-emerald-950 placeholder:text-emerald-900/20 focus:ring-emerald-500/20'}`}
                />
                <input 
                  type="password" 
                  ...
                  className={`w-full border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 transition-all ${loginMode === 'staff' ? 'bg-slate-800 text-white placeholder:text-slate-600 focus:ring-emerald-500/20' : 'bg-white border-emerald-100 text-emerald-950 placeholder:text-emerald-900/20 focus:ring-emerald-500/20'}`}
                />
                <button type="submit" className={`w-full py-4 font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-xl ${loginMode === 'staff' ? 'bg-white text-slate-900 hover:bg-slate-200' : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/20'}`}>
                  Initiate Sync
                </button>
              </form>
            </div>
          ...
          <div className="hidden md:flex flex-col gap-6">
            <div className={`p-8 rounded-[2.5rem] border bg-white/40 border-emerald-100 text-left transition-all ${loginMode === 'staff' ? 'opacity-40 grayscale' : 'opacity-100 scale-105 shadow-xl shadow-emerald-900/5'}`}>
               ...
               <p className="text-sm text-emerald-900/60 leading-relaxed">Patients use Social Profiles (Google/Apple) to link their health history to the NEXIS Intelligence network instantly.</p>
            </div>
        ...
        <p className="text-[10px] text-emerald-800/40 font-bold uppercase tracking-widest mt-12 animate-pulse">
