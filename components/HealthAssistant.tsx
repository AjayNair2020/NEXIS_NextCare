...
const HealthAssistant: React.FC = () => {
  const [messages, setMessages] = useState<EnhancedMessage[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! I am NextCare AI (NEXIS Core). I am now connected to the global medical search network. How can I assist you with clinical insights, document analysis, or anatomical illustrations today?',
      timestamp: new Date(),
    }
  ]);
  ...
  const handleSend = async (e?: React.FormEvent, isVisualGuideRequest: boolean = false) => {
    ...
    let isForcedIllustration = isVisualGuideRequest;

    // Command parsing
    if (textToProcess.startsWith('/anatomy ')) {
      textToProcess = textToProcess.replace('/anatomy ', '');
      isForcedIllustration = true;
    }

    if ((!textToProcess && !attachedFile) || isLoading) return;

    if (isForcedIllustration) {
      await ensureApiKey();
    }

    let finalInput = textToProcess;
    if (attachedFile && !textToProcess) {
    ...
    } else if (isForcedIllustration) {
      finalInput = `Provide an anatomical diagram and explanation for: ${textToProcess}`;
    }
    ...
    try {
      ...
      if (isForcedIllustration) {
        illustrationUrl = await generateMedicalIllustration(textToProcess);
      }
  ...
  return (
    <div className={`max-w-5xl mx-auto h-[calc(100vh-120px)] flex flex-col rounded-3xl shadow-2xl border overflow-hidden relative transition-colors duration-500 ${isDark ? 'bg-[#1b263b] border-blue-900/30' : 'bg-white border-slate-100'}`}>
      <div className={`p-5 border-b flex items-center justify-between transition-colors ${isDark ? 'bg-[#111d2b] border-blue-900/40 text-white' : 'bg-slate-900 border-slate-100 text-white'}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase text-amber-400 tracking-widest">Advanced Multimodal Synth • Online</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
           <button onClick={() => setMessages([messages[0]])} className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 border rounded-xl transition-all ${isDark ? 'text-blue-400 border-blue-800 hover:text-white hover:bg-blue-800/20' : 'text-slate-400 border-slate-700 hover:text-white hover:bg-slate-800'}`}>Reset Link</button>
        </div>
      </div>

      <div ref={scrollRef} className={`flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar ${isDark ? 'bg-[#0f172a]/20' : 'bg-slate-50/30'}`}>
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            <div className={`max-w-[85%] flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-10 h-10 rounded-2xl flex-shrink-0 flex items-center justify-center shadow-lg transition-transform hover:scale-110 ${msg.role === 'user' ? 'bg-blue-600 text-white' : (isDark ? 'bg-[#111d2b] text-amber-400 border border-blue-900/40' : 'bg-slate-900 text-amber-400')}`}>
              ...
              <div className={`flex flex-col gap-3 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                {msg.file && (
                  <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[10px] font-black uppercase text-amber-500 mb-1">
                ...
                {msg.illustrationUrl && (
                  <div className={`rounded-[2rem] overflow-hidden border shadow-2xl max-w-lg transition-all hover:scale-[1.02] ${isDark ? 'border-blue-900/40' : 'border-slate-100'}`}>
                    <img src={msg.illustrationUrl} alt="Clinical Illustration" className="w-full h-auto" />
                    <div className="bg-slate-900 px-4 py-3 text-[10px] font-black uppercase text-amber-400 tracking-widest text-center border-t border-white/5">
                ...
                {msg.videoUrl && (
                  <div className={`rounded-[2rem] overflow-hidden border shadow-2xl max-w-xl transition-all hover:scale-[1.02] ${isDark ? 'border-blue-900/40' : 'border-slate-100'}`}>
                ...
                <div className={`p-6 rounded-3xl shadow-lg text-sm leading-relaxed ${msg.role === 'user' ? 'bg-blue-600 text-white font-medium' : (isDark ? 'bg-[#23324a] text-blue-50 border border-blue-800' : 'bg-white text-slate-800 border border-slate-100')}`}>
                  <div className="prose prose-sm max-w-none whitespace-pre-wrap dark:prose-invert">{msg.content}</div>
                  {msg.sources && (
                    <div className={`mt-4 pt-4 border-t flex flex-wrap gap-2 ${isDark ? 'border-blue-900/30' : 'border-slate-50'}`}>
                       <p className="w-full text-[9px] font-black text-slate-400 uppercase mb-1">Knowledge Grounding</p>
                       {msg.sources.map((s, i) => (
                         <a key={i} href={s.uri} target="_blank" rel="noopener noreferrer" className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${isDark ? 'bg-blue-900/40 border border-blue-800 text-blue-400 hover:text-white' : 'bg-slate-50 border border-slate-100 text-blue-600 hover:bg-blue-50'}`}>
                           {s.title}
                         </a>
                       ))}
                    </div>
      ...
      <div className={`p-6 border-t space-y-4 relative transition-colors ${isDark ? 'bg-[#111d2b] border-blue-900/40' : 'bg-white border-slate-100'}`}>
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
           ...
           <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${isDark ? 'bg-amber-500/10 text-amber-500 border-amber-500/20 hover:bg-amber-500 hover:text-white' : 'bg-amber-50 text-amber-600 border-amber-100 hover:bg-amber-600 hover:text-white'}`}
           >
           ...
           <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap disabled:opacity-50 ${isDark ? 'bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-600 hover:text-white' : 'bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-600 hover:text-white'}`}
           >
           ...
           <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap disabled:opacity-50 ${isDark ? 'bg-purple-500/10 text-purple-400 border-purple-500/20 hover:bg-purple-600 hover:text-white' : 'bg-purple-50 text-purple-600 border-purple-100 hover:bg-purple-600 hover:text-white'}`}
           >
        ...
        {attachedFile && (
          <div className={`flex items-center justify-between p-3 rounded-2xl animate-in slide-in-from-left duration-300 ${isDark ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-amber-50 border border-amber-100'}`}>
             <div className="flex items-center gap-3">
               ...
               <div>
                  <p className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-amber-400' : 'text-amber-800'}`}>Ready for Analysis</p>
                  <p className={`text-xs font-bold ${isDark ? 'text-amber-500' : 'text-amber-600'}`}>{attachedFile.name}</p>
               </div>
             </div>
        ...
        <form onSubmit={(e) => handleSend(e)} className="flex gap-3 items-center">
          <button 
            ...
            className={`p-4 rounded-2xl transition-all shadow-lg ${isListening ? 'bg-rose-500 text-white animate-pulse' : (isDark ? 'bg-[#23324a] text-blue-400 border border-blue-800' : 'bg-slate-100 text-slate-400 hover:bg-slate-200')}`}
          >
          ...
          <input 
            ...
            placeholder="Type symptoms or use '/anatomy [symptom]'..." 
            className={`flex-1 border-none focus:ring-2 rounded-2xl py-4 px-6 text-sm font-medium outline-none transition-all shadow-inner ${isDark ? 'bg-[#020617] text-white placeholder:text-blue-900/50 focus:ring-blue-500/20' : 'bg-slate-50 text-slate-800 placeholder:text-slate-400 focus:ring-amber-500/20'}`}
          />
          <div className="flex gap-2">
            <button 
              type="submit" 
              disabled={(!input.trim() && !attachedFile) || isLoading} 
              className="p-4 bg-amber-500 text-white rounded-2xl hover:bg-amber-600 shadow-xl shadow-amber-500/20 disabled:opacity-50 transition-all active:scale-95"
            >
        ...
        <div className="flex justify-center gap-6 mt-1">
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5 group cursor-help">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5 group cursor-help">
            ...
            Image Synth Eng: v4.2
          </span>
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5 group cursor-help">
            <div className="w-1 h-1 bg-emerald-500 rounded-full group-hover:scale-150 transition-transform"></div>
            Search Grounding Connected
          </span>
        </div>
      </div>
    </div>
