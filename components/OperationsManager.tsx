...
const OperationsManager: React.FC = () => {
  const [activeView, setActiveView] = useState<'inventory' | 'transports' | 'services' | 'compliance' | 'safety'>('inventory');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-white/50 backdrop-blur-md p-1 rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
          {['inventory', 'transports', 'services', 'compliance', 'safety'].map(view => (
            <button 
              key={view}
              onClick={() => setActiveView(view as any)}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                activeView === view 
                  ? (view === 'safety' ? 'bg-blue-600 text-white shadow-lg' : 'bg-emerald-600 text-white shadow-lg') 
                  : 'text-slate-500 hover:text-emerald-600'
              }`}
            >
              {view === 'safety' ? 'AI Safety' : view}
            </button>
          ))}
        </div>
      </div>
      ...
      {activeView === 'safety' ? (
        <AISafetyIntelligence isDarkMode={isDark} />
      ) : (
        <>
